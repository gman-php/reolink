...
class ReolinkApi(object):
        ...
        for data in json_data:
            try:
                if data["cmd"] == "GetDevInfo":
                    ...
                    detectionZone = 1
                    if "scope" in data["value"]["Alarm"] and "table" in data["value"]["Alarm"]["scope"]:
                        detectionZone = int("".join(set(data["value"]["Alarm"]["scope"]["table"])))

                    if data["value"]["Alarm"]["enable"] >= 1 and detectionZone > 0:
    ...
    async def set_motion_detection(self, enabled):
        ...
        if "scope" in body[0]["param"]["Alarm"] and "table" in body[0]["param"]["Alarm"]["scope"]:
            body[0]["param"]["Alarm"]["scope"]["table"] = body[0]["param"]["Alarm"]["scope"]["table"].replace(str(int(not newValue)), str(newValue))
